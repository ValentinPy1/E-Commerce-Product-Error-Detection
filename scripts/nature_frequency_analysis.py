...
def main():
    ...
    def calculate_error_rates_by_nature(df, model_names):
        ...
        for min_freq, max_freq, bin_label in freq_bins:
            ...
            if max_freq == float('inf'):
                ...
                mask = (nature_counts >= min_freq) & (nature_counts <= max_freq)
    ...
    plt.subplot(2, 2, 1)
    pivot_data = error_analysis.pivot(index='frequency_bin', columns='model', values='error_rate')
    ...
    plt.subplot(2, 2, 2)
    sample_counts = error_analysis.groupby('frequency_bin')['total_samples'].first()
    ...
    for model in model_names:
        ...
        plt.scatter(model_data['avg_frequency'], model_data['error_rate'], 
    ...
    plt.subplot(2, 2, 4)
    heatmap_data = error_analysis.pivot(index='frequency_bin', columns='model', values='error_rate')
    ...
    plt.savefig('artifacts/nature_frequency_error_analysis.png', dpi=300, bbox_inches='tight')
    ...
    plt.subplot(2, 1, 1)
    pivot_detailed = detailed_df.pivot(index='nature', columns='model', values='error_rate')
    
    # Sort nature labels by frequency (most frequent first)
    nature_freq_order = detailed_df.groupby('nature')['frequency'].first().sort_values(ascending=False).index
    ...
    for model in model_names:
        ...
        plt.scatter(model_data['frequency'], model_data['error_rate'], 
                    label=model, alpha=0.7, s=100)
    ...
    plt.savefig('artifacts/detailed_nature_error_analysis.png', dpi=300, bbox_inches='tight')
    ...
    for model in model_names:
        model_data = detailed_df[detailed_df['model'] == model]
        if len(model_data) > 1:
            correlation = model_data['frequency'].corr(model_data['error_rate'])
            print(f"{model}: {correlation:.4f}")
    
    # Summary statistics by frequency bin
    print("\nSummary Statistics by Frequency Bin:")
    summary_stats = error_analysis.groupby('frequency_bin').agg({
    ...
    detailed_df.to_csv('artifacts/nature_frequency_detailed_analysis.csv', index=False)
    error_analysis.to_csv('artifacts/nature_frequency_error_analysis.csv', index=False)
    ...
    for bin_name in freq_order:
        ...
        if len(bin_data) > 0:
            ...
            print(f"\n{bin_name}:")
            print(f"  Best: {best_model['model']} (Error Rate: {best_model['error_rate']:.3f})")
            print(f"  Worst: {worst_model['model']} (Error Rate: {worst_model['error_rate']:.3f})")
    
    # Find nature labels with highest error rates across all models
    print("\nNature Labels with Highest Error Rates (Top 10):")
    avg_errors = detailed_df.groupby('nature')['error_rate'].mean().sort_values(ascending=False)
    ...
    freq_ordered = detailed_df.groupby('nature')['frequency'].first().sort_values(ascending=False)
