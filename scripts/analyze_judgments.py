...
def calculate_confidence_thresholds(
    judgments: Dict[str, pd.DataFrame],
    ...
    for model_name, df in judgments.items():
        ...
        if len(valid) < 10:
            print(
                f"  Skipping {model_name}: insufficient samples ({len(valid)} < 10)")
            continue
        ...
        print(
            f"  Overall accuracy in disagreements: {overall_accuracy:.3f} ({model_correct_total}/{len(valid)})")

        # Create confidence bins
        valid["conf_bin"] = pd.cut(
            valid["model_conf"], bins=10, include_lowest=True)
        ...
        for bin_name, bin_data in valid.groupby("conf_bin", observed=False):
            if len(bin_data) >= min_samples_per_bin:
                ...
                print(
                    f"    Bin {bin_name}: conf={conf_mean:.3f}, accuracy={accuracy:.3f}, count={total}")
        ...
        if len(bin_df) >= 2:
            try:
                ...
                if reg.coef_[0] != 0:  # Avoid division by zero
                    ...
                    print(
                        f"  Found threshold via linear regression: {threshold:.3f}")

                    # Check if threshold is within reasonable range
                    if threshold < 0 or threshold > 1:
                        print(
                            f"  Warning: Threshold {threshold:.3f} outside [0,1] range, using linear interpolation")
                        threshold = None
                        threshold_method = None
        ...
        if threshold is None:
            for i in range(len(bin_df) - 1):
                if (bin_df.iloc[i]["accuracy"] <= target_accuracy and
                    ...
                    threshold = conf1 + \
                        (target_accuracy - acc1) * \
                        (conf2 - conf1) / (acc2 - acc1)
                    threshold_method = "linear_interpolation"
                    print(
                        f"  Found threshold via linear interpolation: {threshold:.3f}")
                    break
        ...
        if threshold is None and len(bin_df) >= 3:
            try:
                ...
                if len(np.unique(y)) > 1:  # Need both classes
                    ...
                    print(
                        f"  Found threshold via logistic regression: {threshold:.3f}")
            except Exception as e:
        ...
        if threshold is None:
            ...
            if not above_target.empty:
                ...
                print(
                    f"  Found threshold via first above target: {threshold:.3f}")
            else:
...
def plot_confidence_analysis(judgments: Dict[str, pd.DataFrame],
    ...
    for model_name in all_models:
            ...
            if len(bin_data) >= 3:
                ...
                if total > 0 and accuracy > 0 and accuracy < 1:
                    ...
                    centre_adjusted_probability = (
                        accuracy + z*z/(2*total))/denominator
                    adjusted_standard_error = z * \
                        np.sqrt((accuracy * (1 - accuracy) + z *
                                z/(4*total))/total)/denominator
                    lower_bound = max(
                        0, centre_adjusted_probability - adjusted_standard_error)
                    upper_bound = min(
                        1, centre_adjusted_probability + adjusted_standard_error)
                else:
        ...
        if len(all_confidences) >= 2:
            ...
            X_plot = np.linspace(min(all_confidences), max(
                all_confidences), 100).reshape(-1, 1)
    ...
    summary_path = os.path.join(
        output_dir, "confidence_thresholds_summary.csv")
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze LLM judgments to find confidence thresholds")
    parser.add_argument("--judgments_dir", type=str,
                        default="./artifacts/llm_judgments")
    parser.add_argument("--output_dir", type=str,
                        default="./artifacts/confidence_analysis")
    ...
    summary_df = plot_confidence_analysis(
        judgments, thresholds, args.output_dir)
    ...
    for _, row in summary_df.iterrows():
        print(f"  {row['model']}:")
        print(
            f"    Overall accuracy in disagreements: {row['overall_accuracy_pct']} ({row['model_correct_count']}/{row['total_samples']})")
        if pd.notna(row["threshold"]):
            print(
                f"    Confidence threshold: {row['threshold']:.3f} ({row['threshold_method']})")
            print(f"    → Trust model above {row['threshold']:.1%} confidence")
        else:
            print(
                f"    Confidence threshold: None (never right >{args.target_accuracy:.0%} when disagreeing)")
        print()
